        ...
        <v-row class="fill-height" data-app='true'>
			<v-col>
				<v-sheet height="64">
					...
					<v-btn class='mr-4' large dark color='#17a2b8' @click.stop='dialog = true'>
						預約維修服務
					</v-btn>
					...
					>
						<v-container>
							...
							<v-text-field
								filled
								dense
								v-model="start" type="date" label="日期"
							></v-text-field>
							<p class='m-2'>預約時間</p>
							<v-time-picker
								color='#007bff'
							...
							<v-text-field 
								filled
								dense
								v-model="color" type="color" label="顏色"
							></v-text-field>
							<v-btn type="submit" outlined dark color='#17a2b8' @click.stop="dialog = false">
								新增預約 <span style="font-style: italic;">(系統將寄送預約成功電子郵件到您的信箱)</span>
							</v-btn>
							</v-form>
						</v-container>
					</v-card>
...
export default {
    ...
    methods: {
        ...
        async updateEvent (ev) {
            await db.collection('rsv-cal').doc(this.currentlyEditing).update({
                email: ev.email
            })
        ...
        async deleteEvent (ev) {
            await db.collection("rsv-cal").doc(ev).delete().then(function() {
